...
def call_cnvs_lite(cfdna_object, bins, file_name, bin_size=100000, bin_bias_h5_fn=None, method="bcp_online_both",
    ...
    cnv_bins, cnv_segs = ngs.segment.cnv.call_cnvs(bins, bin_size=bin_size, bin_bias_h5_fn=bin_bias_h5_fn,
...
def predict_purity_lite(cfdna_object, bins, file_name, bin_size = 1000000, bin_bias_h5_fn=None, method="bcp_online_both",
...
def train_hmm_lite(cfdna_object, bins, file_name, bin_size = 1000000, bin_bias_h5_fn=None, method="bcp_online_both",
                outlier_smooth=True, gauss_smooth=False, bcp_cutoff=0.3, normal = [0.1, 0.5, 0.9],
    ...
    # Calculate bins
    bins, segments = ngs.segment.cnv.call_cnvs(bins, bin_size=bin_size, bin_bias_h5_fn=bin_bias_h5_fn, genome=cfdna_object.ref_genome, method=method,
                                outlier_smooth=outlier_smooth, gauss_smooth=gauss_smooth, verbose=verbose, cutoff=bcp_cutoff)
...
def process_cnvs_lite(cfdna_object, frags=None, file_name=None, merge=True):
    """
    """
...
def call_cnv_pipline_lite(sam_fn, cnv_binsize=100000, hmm_binsize=1000000, method="bcp_online_both", outlier_smooth=True,
    ...
    cfdna_object.filenames.add(file_name)
    ...
    # Train HMM
    hmm_bins = ngs.read_sam.ReadSam.read_bin_fragments(sam_fn, hmm_binsize)
    train_hmm_lite(cfdna_object, hmm_bins, file_name, bin_size = hmm_binsize, bin_bias_h5_fn=None, method=method,
                outlier_smooth=outlier_smooth, gauss_smooth=gauss_smooth, bcp_cutoff=bcp_cutoff, normal=normal,
                ploidy=ploidy, estimatePloidy=estimatePloidy, minSegmentBins=minSegmentBins, maxCN=maxCN, verbose=verbose)

    # Call Copy Number Variations
    cnv_bins = ngs.read_sam.ReadSam.read_bin_fragments(sam_fn, cnv_binsize)
    call_cnvs_lite(cfdna_object, cnv_bins, file_name, bin_size=cnv_binsize, bin_bias_h5_fn=None, method=method,
                outlier_smooth=outlier_smooth, gauss_smooth=gauss_smooth, verbose=verbose, cutoff=bcp_cutoff)
    process_cnvs_lite(cfdna_object, file_name=file_name)
